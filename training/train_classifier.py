...
def _load_dataframe(csv_path: str) -> pd.DataFrame:
    ...
    if "complaint_text" not in df.columns or "issue_type" not in df.columns:
        raise ValueError("CSV must contain 'complaint_text' and 'issue_type' columns")
    df = df.dropna(subset=["complaint_text", "issue_type"]).reset_index(drop=True)
    return df


def _encode_labels(labels: List[str]) -> Dict[str, int]:
    unique = sorted(set(labels))
    return {label: idx for idx, label in enumerate(unique)}


def _tokenize_function(tokenizer, texts: List[str], max_length: int) -> Dict[str, np.ndarray]:
...
def main() -> None:
    ...
    parser.add_argument("--model", default="bert-base-uncased", help="Pretrained checkpoint")
    parser.add_argument("--output", default=str(PATHS.classifier_dir), help="Directory to save the fine-tuned model")
    ...
    label_map = _encode_labels(df["issue_type"].tolist())
    ...
    def tokenize_batch(batch):
        return _tokenize_function(tokenizer, batch["complaint_text"], args.max_length)
